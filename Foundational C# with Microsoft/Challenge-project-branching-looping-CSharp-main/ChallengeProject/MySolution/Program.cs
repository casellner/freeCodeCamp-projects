...
do
{
    ...
    switch (menuSelection)
    {
        ...
        case "3":
            ...
            for (int i = 0; i < maxPets; i++)
            {
                if (ourAnimals[i, 0] != "ID #: ") // if ID is not default value
                {
                    if (ourAnimals[i, 2] == "Age: " || ourAnimals[i, 2] == "Age: ?") // if age is missing or incomplete
                    {
                        do
                        {
                            ...
                            if (readResult != null)
                            {
                                ...
                                validEntry = int.TryParse(animalAge, out petAge);
                                if (validEntry) {
                                    if (ourAnimals[i, 2] == "Age: ?") //if the question mark needs to be removed
                                    {
                                        ourAnimals[i, 2] = ourAnimals[i, 2].Remove(ourAnimals[i, 2].Length - 1, 1) + petAge;
                                    }
                                    else
                                    {
                                        ourAnimals[i, 2] += petAge;
                                    }
                                }
                            }
                        } while (validEntry == false);
                    ...
                    if (ourAnimals[i, 4] == "Physical description: ")
                    {
                        do
                        {
                            Console.WriteLine($"Enter an physical description for {ourAnimals[i, 0]} (size, color, gender, weight, housebroken)");
                            readResult = Console.ReadLine();
                            if (readResult != null)
                            {
